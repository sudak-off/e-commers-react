...
function App() {
	...
	const onAddToCart = async (obj) => {
		try {
			...
			if (findItem) {
				...
				await axios.delete(
					`https://622a55b914ccb950d21a16d7.mockapi.io/Cart/${findItem.id}`
				);
			} else {
				setCartItems((prev) => [...prev, obj]);
				const { data } = await axios.post(
					"https://622a55b914ccb950d21a16d7.mockapi.io/Cart",
					obj
				);
				setCartItems((prev) =>
					prev.map((item) => {
	...
	const onRemoveItem = async (id) => {
		try {
			await axios.delete(
				`https://622a55b914ccb950d21a16d7.mockapi.io/Cart/${id}`
			);
			setCartItems((prev) =>
	...
	const onAddToFavorite = async (obj) => {
		try {
			if (favorites.find((favObj) => Number(favObj.id) === Number(obj.id))) {
				axios.delete(
					`https://622a55b914ccb950d21a16d7.mockapi.io/favorites/${obj.id}`
				);
				setFavorites((prev) =>
				...
				const { data } = await axios.post(
					"https://622a55b914ccb950d21a16d7.mockapi.io/favorites",
					obj
				);
